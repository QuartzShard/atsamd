...
impl<S: Sercom> Registers<S> {
    ...
    #[inline]
    pub(in super::super) fn i2c_master(&self) -> &pac::sercom0::I2cm {
        self.sercom.i2cm()
    }
    ...
    #[inline]
    pub(super) fn set_op_mode(&mut self, mode: pac::sercom0::i2cm::ctrla::Modeselect) {
        self.i2c_master()
            .ctrla()
    ...
    pub(super) fn write_one(&mut self, byte: u8) {
        unsafe {
            self.i2c_master().data().write(|w| w.bits(byte as DataReg));
        }
    }
